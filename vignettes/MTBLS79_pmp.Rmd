...
To work with `structToolbox` object the data must be converted from `SummarisedExperiment` to `DatasetExperiment`. Some additional meta-data columns are also needed to carry out certain filters.
...
cat(paste0('Number of features removed: ', nc))
```
Finally an RSD filter is used to remove with high analytical variation (QC RSD > 20 removed)
