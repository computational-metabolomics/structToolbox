...
setMethod(f="chart_plot",
          ...
          definition=function(obj,dobj)
          {
              opt=param_list(obj)
              A=data.frame(
                  x=output_value(dobj,'correlation')[,opt$components[1]],
                  y=output_value(dobj,'correlation')[,opt$components[2]])
              dat <- circleFun(c(0,0),2,npoints = 50)
              
              out=ggplot(data=A,aes(x=.data[['x']],y=.data[['y']])) +
                  geom_point() +
                  scale_colour_Publication() +
                  theme_Publication(base_size = 12)+
                  coord_fixed(xlim = c(-1,1),ylim=c(-1,1)) +
                  
                  geom_path(data=dat,aes(
                      x=.data[['x']],
                      y=.data[['y']]),
                      inherit.aes = FALSE)
...
setMethod(f="chart_plot",
          ...
          definition=function(obj,dobj)
          {
              ...
              {
                  out=out+
                      geom_point(data=A,inherit.aes = FALSE,color='black',
              ...
              if (opt$style=='arrows')
              {
                  out=out+
                      ...
                      geom_segment(data=A,inherit.aes = FALSE,color='black',
                                   mapping = aes(
                                       x = 0,
                                       y = 0,
                                       xend = .data[['x']],
...
setMethod(f="chart_plot",
          ...
          definition=function(obj,dobj)
          {
              ...
              A=data.frame(
                  "x"=1:length(pvar),
                  "y"=c(pvar,cumsum(pvar)),
                  "Variance"=as.factor(c(
                      rep('Single component',length(pvar)),
                      rep('Cumulative',length(pvar)))))
...
setMethod(f="chart_plot",
          ...
          definition=function(obj,dobj)
          {
              ...
              out=ggplot(data=df, aes(x=.data[['x']],y=.data[['y']])) +
