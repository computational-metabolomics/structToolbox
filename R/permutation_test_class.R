              ...
              if (obj$style=='boxplot') {
                  ...
                  out=ggplot(data=A,
                             aes(
                                 ...
                                 color=.data[['dataset']])) +
                      scale_color_manual(
                          values=plotClass$manual_colors,
                          name='Dataset') +
                      geom_violin(trim=F) +
                      theme(legend.position="none") +xlab('Dataset')
                  
              } else if (obj$style=='histogram') {
                  out=ggplot(data=A,aes(x=.data[['value']],color=.data[['dataset']])) +
                      scale_color_manual(values=plotClass$manual_colors,name='Dataset') +
                      geom_freqpoly(binwidth = obj$binwidth)
                  
              } else if (obj$style=='scatter') {
                  out=ggplot(data=A,aes(x=1:nrow(A),y=.data[['value']],color=.data[['dataset']])) +
                      geom_point(na.rm=T) +
                      scale_color_manual(values=plotClass$manual_colors,name='Dataset') +
                  ...
                  out=ggplot(data=A,aes(x=.data[['value']],color=.data[['dataset']],fill=.data[['dataset']])) +
                      scale_color_manual(values=plotClass$manual_colors,name='Dataset') +
