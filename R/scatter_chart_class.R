...
setMethod(f="chart_plot",
          ...
          definition=function(obj,dobj)
          {
                      ...
                      out = out +
                          stat_ellipse(
                              ...
                              aes(
                                  ...
                                  colour=.data[['group']],
                                  group=.data[['group']]),
                              type = obj$ellipse_type,
                              level=obj$ellipse_confidence) # ellipse for 
                      # individual groups
                  } else {
                      if (is.factor(shapes)) {
                          out = out +stat_ellipse(
                              data=A, 
                              aes(
                                  x=.data[['x']],
                                  y=.data[['y']],
              ...
              if (is(groups,'factor')) { # if a factor then plot by group using the colours from pmp package
                  ...
                  out=out+scale_colour_viridis_c(
                      limits=quantile(
                          groups,
                          c(0.05,0.95),na.rm = TRUE),
                      oob=scales::squish,name=obj$factor_name[[1]])
              }
              ...
              if (obj$ellipse %in% c('all','sample')) { # only do this if 
                                                # we plotted the sample ellipse
                  ...
                  points$in.ell=as.logical(
                      sp::point.in.polygon(points$x,points$y,ell$x,ell$y))
